...
export default function UniversitiesPage() {
  ...
  const isSuperAdmin = session?.user?.role === "SUPER_ADMIN" || session?.user?.role === "UNIVERSITY_ADMIN";
    ...
    <div className="space-y-6 p-6 pt-20 max-w-6xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <PageHeader
          icon={Globe}
          title="Universities"
          description={`Browse ${total} universities — QS rankings shown where available`}
        />
        {isSuperAdmin && (
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-12">QS Rank</TableHead>
                  <TableHead>Name</TableHead>
                  <TableHead>Country</TableHead>
                  <TableHead>QS Overall</TableHead>
                  <TableHead>QS Academic</TableHead>
                  <TableHead>QS Employer</TableHead>
                  <TableHead>QS Faculty</TableHead>
                  <TableHead>QS Citations</TableHead>
              ...
              <TableBody>
                {universities.length === 0 ? (
                    <TableRow>
                      ...
                      {isSuperAdmin && " Click \"Import from API\" to load the global university database."}
                    </TableCell>
                  </TableRow>
                ) : universities.map((u) => (
                  <TableRow key={u.id}>
                    ...
                    <TableCell className="max-w-[160px]">
                      {u.domains?.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {u.domains.slice(0, 2).map((d: string) => (
                            <span key={d} className="text-xs bg-muted px-1.5 py-0.5 rounded">{d}</span>
                          ))}
                          {u.domains.length > 2 && <span className="text-xs text-muted-foreground">+{u.domains.length - 2}</span>}
                        </div>
                      ) : "—"}
                    </TableCell>
